...
import StatusChangeModal from './Common/Modal/StatusChangeModal';
    ...
    <div className='flex flex-col items-center'>
      ...
      <caption className="caption-top text-2xl">
        ...
        <thead>
          ...
          <th className='border border-slate-600 p-4'>Case Type</th>
          <th className='border border-slate-600 p-4'>Case Category</th>
          <th className='border border-slate-600 p-4'>Case Number</th>
          <th className='border border-slate-600 p-4'>Section</th>
          <th className='border border-slate-600 p-4'>First Party</th>
          <th className='border border-slate-600 p-4'>Second Party</th>
          <th className='border border-slate-600 p-4'>Registration Date</th>
          <th className='border border-slate-600 p-4'>Age of the Case</th>
              ...
              <tr key={index}>
                ...
                <td className='border border-slate-600 p-5 text-center'>
                  ...
                  <button className='dark:bg-slate-700 bg-slate-300 p-2 mt-1 hover:bg-red-500'>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 fill-none hover:fill-white">
                      <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                    </svg>
                  </button> 
                  <button className='dark:bg-slate-700 bg-slate-300 p-2 mt-1 hover:bg-blue-500' onClick={()=>showStatusChangeData(data.id)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 fill-none hover:fill-white">
                      <path strokeLinecap="round" strokeLinejoin="round" d="m11.25 11.25.041-.02a.75.75 0 0 1 1.063.852l-.708 2.836a.75.75 0 0 0 1.063.853l.041-.021M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9-3.75h.008v.008H12V8.25Z" />
                    </svg>
                  </button> 
                  {statusChangeModalOpen && <StatusChangeModal closeModal={()=>setStatusChangeModalOpen(false)} dataId={dataId}/>}
                </td>
              </tr>
          ))} 
        </tbody>
