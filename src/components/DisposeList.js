...
const GET_CASES_URL = 'http://localhost:5050/api/v1/cases/disposed';
...
const DisposedList = () => {
  ...
  return (
    ...
    <div className='dark:text-white text-slate-700 text-2xl mt-4'>
      No disposed records present in the database to display
    </div> : 
    ...
    <caption className="caption-top text-2xl">
        Disposed case details.
    </caption>
      <thead>
        <th className='border border-slate-600 p-5'>SL No</th>
      ...
      <tbody>
        {caseData.map((data,index) => ( 
            <tr key={index}>
              ...
              <td className='border border-slate-600 p-5 text-center'>{data.caseCategory} {data.caseNumber}</td>
